...
# MODIFIED ADDING SCRIPT TO EXTRACT MAX-MIN INTENSITY DIFFERENCE BY PRIYANKOO SARMAH ON MARCH 15, 2013 
# ########################################################################################################################### #

# COLLECT ALL THE USER INPUT
form Select directories for TextGrids and Sound files
	sentence Textgrid_directory /Users/Jennifer/Desktop/1006/TextGrid/
	sentence Sound_directory /Users/Jennifer/Desktop/1006/
	...
	real Zoom_duration 10
	comment Full path of the output file:
	sentence Output_file /Users/Jennifer/Desktop/1006.xls
	...
	positive Default_max_formant 3000
	integer Default_formant_number 3
	real Time_step 0.1
		...
		if label$ <> ""
			...
			if clicked = 3
				...
				for i from 1 to total_interval_measurements
					...
					f3 = 0
					# WRITE TO FILE
					resultline$ = "'current_file''tab$''filename$''tab$''label$''tab$''percent:0''tab$''time''tab$''f1''tab$''f2''tab$''f3''tab$''f2f1''tab$''max_formant''tab$''formant_number''tab$''notes_or_comments$''newline$'"
			...
			# IF THE USER ACCEPTS THE FORMANT VALUES...
				...
				for i from 1 to total_interval_measurements
					time = intrvl_meas_time [i]
					percent = ((time-start)/(end-start))*100
					f1h = Get value at time... 1 time Hertz Linear
					f2h = Get value at time... 2 time Hertz Linear
					f3h = Get value at time... 3 time Hertz Linear
					f1 = hertzToMel(f1h)
					f2 = hertzToMel(f2h)
					f3 = hertzToMel(f3h)
					f2f1 = (f2-f1)
...
procedure initializeOutfile
	headerline$ = "number'tab$'filename'tab$'label'tab$'percent through interval'tab$'measurement time'tab$'F1'tab$'F2'tab$'F3'tab$'F2-F1'tab$'max formant'tab$'number of formants'tab$'notes'newline$'"
	fileappend "'output_file$'" 'headerline$'
endproc
#joining script intDifference max_min intensity
directory$=sound_directory$
fileappend "'output_file$'_intensity.xls"  Filename'tab$'DurationV1'tab$'DurationV2'tab$'Intensity Difference'tab$''newline$'
...
Read from file... 'directory$'/TextGrid/'fileName$'.TextGrid
	...
	if label$ <> ""
	v1_start=Get starting point... 1 i
	v1_end=Get end point... 1 i
	v2_start=Get starting point... 1 i+2
	v2_end=Get end point... 1 i+2
	select Sound 'fileName$'
	To Intensity... 100 0 yes
	max_v1 = Get maximum... v1_start v1_end Parabolic
	max_v2 = Get maximum... v2_start v2_end Parabolic
	#print 'max_v1' 'newline$' 'max_v2' 'newline$''v2_start''newline$''v2_end''newline$'
	intDiff = max_v1 - max_v2
	duration2 = (v2_end - v2_start)*1000
	duration1 = (v1_end - v1_start)*1000
	fileappend "'output_file$'_intensity.xls" 'fileName$''tab$''duration1:3''tab$''duration2:3''tab$''intDiff:3''tab$''newline$'
endif
endfor
